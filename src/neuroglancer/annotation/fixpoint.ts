...
import {AnnotationType, Fixpoint} from 'neuroglancer/annotation';
...
void setFixpointMarkerColor(vec4 color) {
  vColor = color;
}
void setFixpointMarkerBorderColor(vec4 color) {
  vBorderColor = color;
}
`);
...
`);
  ...
  private shaderGetter3d =
      this.getDependentShader('annotation/fixpoint:3d', (builder: ShaderBuilder) => {
...
`);
  ...
  private makeShaderGetter2d = (extraDim: number) =>
      this.getDependentShader(`annotation/fixpoint:2d:${extraDim}`, (builder: ShaderBuilder) => {
        defineVertexId(builder);
...
registerAnnotationTypeRenderHandler<Fixpoint>(AnnotationType.FIXPOINT, {
...
`);
  ...
  getRepresentativePoint(out, ann) {
    out.set(ann.fixpoint);
  },
  updateViaRepresentativePoint(oldAnnotation, position) {
    return {...oldAnnotation, fixpoint: new Float32Array(position)};
  }
});
